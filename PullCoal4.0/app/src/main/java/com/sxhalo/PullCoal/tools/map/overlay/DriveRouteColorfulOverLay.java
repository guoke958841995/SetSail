package com.sxhalo.PullCoal.tools.map.overlay;
...
public class DriveRouteColorfulOverLay {
     ...
     * 添加驾车路线添加到地图上显示。
     */
    public void addToMap(int DriveColor) {
        initPolylineOptions(DriveColor);
        try {
            if (mAMap == null) {
                return;
            }
     ...
     * 初始化线段属性
     */
    private void initPolylineOptions(int DriveColor) {

        mPolylineOptions = null;
        mPolylineOptions = new PolylineOptions();
        mPolylineOptions.color(DriveColor).width(mWidth);
    }
    ...
    private BitmapDescriptor getDriveBitmapDescriptor() {
        return BitmapDescriptorFactory.fromResource(R.mipmap.custtexture);
    }
    ...
    private int getDriveColor() {
        return Color.parseColor("#5198ee");
    }
