...
public class OrmLiteDBUtil {
	...
	public static void createDb(Context activity){
		try {
			String dbPath = BaseUtils.setDbPath(activity, new Config().DB_NAME);
			DataBaseConfig config = new DataBaseConfig(activity,dbPath);
			config.debugged = false; // open the log
			config.dbVersion = new Config().DB_VERSION; // set database version
			config.onUpdateListener = null; // set database update listener
			liteOrm = LiteOrm.newCascadeInstance(config);// cascade
		} catch (Exception e) {
			GHLog.e("数据库创建失败",e.toString());
			MyException.uploadExceptionToServer(activity,e.fillInStackTrace(),true);
		}
	}
