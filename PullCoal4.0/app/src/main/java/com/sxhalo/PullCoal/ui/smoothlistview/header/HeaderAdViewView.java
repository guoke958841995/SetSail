...
public class HeaderAdViewView extends HeaderViewInterface<List<Slide>> {
    ...
    @Override
    public void getView(List<Slide> list, ListView listView) {
        try {
    ...
    public View getView(List<Slide> list) {
        try {
            view = mInflate.inflate(R.layout.header_ad_layout, null, false);
            ButterKnife.bind(this, view);
    ...
    public void dealWithTheView(List<Slide> list) {
        try {
            ivList.clear();
           final int size = list.size();
            if (size == 2){
                ...
                for (int i = 0; i < size * 2 ; i++) {
                    ivList.add(createImageView(list.get(i % size).getImageUrl()));
                }
            }else{
                for (int i = 0; i < size; i++) {
                    ivList.add(createImageView(list.get(i).getImageUrl()));
                }
            }
