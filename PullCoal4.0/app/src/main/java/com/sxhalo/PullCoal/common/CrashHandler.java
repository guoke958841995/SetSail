...
//            dumpExceptionToSDCard(ex);
    ...
    private void dumpExceptionToSDCard(Throwable ex) throws IOException {
        ...
        if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            if (DEBUG) {
                GHLog.d(TAG, "sdcard unmounted,skip dump exception");
                return;
            }
        }
        ...
        try {
            ...
            GHLog.e(TAG, "dump crash info failed");
        }
    }
    ...
    private void uploadExceptionToServer(Throwable ex) {
        try {
            String crashReport = getCrashReport(MyAppLication.app.getApplicationContext(), ex);
            ...
            params.put("machineIp", BaseUtils.getIPAddress(MyAppLication.app.getApplicationContext())); //当前ip
...
//            AppManager.getAppManager().AppExit(mContext);
    ...
    private Map<String, String> addDeviceType(Map<String, String> params) {
        ...
        if (!"null".equals(BaseUtils.GetDeviceID(mContext))){
            params.put("machineCode", BaseUtils.GetDeviceID(mContext));  //机器码
        }
