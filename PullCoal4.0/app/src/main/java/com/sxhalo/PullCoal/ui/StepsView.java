...
public class StepsView extends View {
     ...
     * 未完成的步骤的颜色
     */
    public static final int COLOR_BAR_UNDONE = 0XFF008DFF;
    /**
     * 完成步骤的颜色
     */
    public static final int COLOR_BAR_DONE = 0XFFceced0;
    /**
     * 未完成步骤的标题颜色
     */
    public static final int COLOR_TITLE_BAR_UNDONE = 0X77aef1;
    /**
     * 完成步骤的标题颜色
     */
    public static final int COLOR_TITLE_BAR_DONE = 0XFFceced0;
    /**
     * 默认线条高度
     */
     ...
     * 默认圆的半径
     */
    public static final int DEFAULT_CIRCLE_RADIUS = 25;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mLeftX = this.getLeft() + getPaddingLeft() + dp2px(30) + DEFAULT_PADDING;
...
//        mCirclePaint.setStyle(Paint.Style.FILL);    //实心圆
        mCirclePaint.setAntiAlias(false);                       //设置画笔为无锯齿
        canvas.drawColor(getResources().getColor(R.color.transparent));
        mCirclePaint.setStrokeWidth((float) 10.0);                    //线宽
        mCirclePaint.setStyle(Paint.Style.STROKE);                   //空心效果
        /*****************初始化圆画笔***************/
        ...
        for (int i = 0; i < titles.length; i++) {
            ...
            if (i < titles.length - 1) {
                canvas.drawRect(mLeftX + i * mDistance + rect.width()+(mRadius/2)-mCirclePaint.getStrokeWidth(), mLeftY - mLineHeight/2, mLeftX + (i + 1) * mDistance-mRadius, mRightY- mLineHeight/2, mLinePaint);
            }

            if (position < titles.length) {
                mLinePaint.setColor(mDoneColor);
                canvas.drawRect(mLeftX + (position - 1) * mDistance + rect.width()+(mRadius/2), mLeftY- mLineHeight/2, mLeftX + (2 * position - 1) * mDistance / 2 + mRadius, mRightY- mLineHeight/2, mLinePaint);
